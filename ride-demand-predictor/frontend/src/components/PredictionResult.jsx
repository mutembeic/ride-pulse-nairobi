      ...
      {result.is_fallback ? (
        <>
          <div className="mb-3 p-2 text-sm text-center bg-yellow-900/50 text-yellow-300 rounded-md">
            No data for the exact spot.
          </div>
          <p className="text-gray-400 text-sm">Recommended Hotspot:</p>
          <p className="text-lg font-semibold text-cyan-300 mb-2">
            {result.fallback_location_name}
          </p>
        </>
      ) : (
        <p className="text-gray-400 text-sm">Predicted Demand at Location:</p>
      )}
